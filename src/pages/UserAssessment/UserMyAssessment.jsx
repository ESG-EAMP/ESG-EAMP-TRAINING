...
import './UserMyAssessment.css';
import Swal from 'sweetalert2';
import MsmeAnalysis from '../UserDashboard/MsmeAnalysis';
import { isVerificationComplete, showVerificationNotification } from '../../utils/verificationUtils';
import {
    Chart as ChartJS,
    CategoryScale,
    LinearScale,
...
function UserMyAssessments() {
    ...
    useEffect(() => {
        // Check verification status and show notification if needed
        const userData = JSON.parse(localStorage.getItem('user'));
        if (!isVerificationComplete(userData)) {
            // Show notification but don't block access
            setTimeout(() => {
                showVerificationNotification(navigate);
        ...
        api.get(`/assessment/user/v2/get-responses-2?user_id=${userId}&selected_only=true`)
            .then(res => {
                ...
                if (Array.isArray(data) && data.length > 0) {
                    setYearlyAssessment(data[0]?.years || []);
                } else {
                    setYearlyAssessment([]);
                }
                setLoading(false);
    ...
    const getOverallScore = (assessment) => {
        ...
        if (assessment.score.total_score !== undefined && assessment.score.max_score !== undefined) {
            return ((assessment.score.total_score / assessment.score.max_score) * 100).toFixed(2);
        }
    ...
    const getCategoryScore = (assessment, category) => {
        ...
        if (assessment.score.category_max && assessment.score.category_max[category] !== undefined) {
            ...
            return ((categoryScore / categoryMax) * 100).toFixed(2);
        }
    ...
    const handleViewDetails = (assessment) => {
        navigate(`/assessment-details-v2/${assessment.year}`);
    };
                ...
                <div className="row align-items-center">
                    <div className="col-md-8">
                        ...
                        <p className="text-white ">
                            Track your ESG assessment progress and view historical results
                        </p>
                    </div>
                    ...
                    <div className="assessment-section">
                        ...
                        {yearlyAssessment.length === 0 ? (
                            <div className="empty-state">
                                ...
                                <span className="btn btn-primary" onClick={() => navigate('/assessment-v2')}>
                                                    ...
                                                    <div className="assessment-info">
                                                        ...
                                                        {item.score && typeof item.score === 'object' && (
                                                            <div className="row gap-1">
                                                                <div className="col-12  ">
                                                                    <span className={` ${getScoreColor(getCategoryScore(item, 'Environment'))}`}>
                                                                       <strong>E:</strong> {getCategoryScore(item, 'Environment') || 'N/A'}% 
                                                                    </span>
                                                                </div>
                                                                <div className="col-12 ">
                                                                    <span className={` ${getScoreColor(getCategoryScore(item, 'Social'))}`}>
                                                                        <strong>S:</strong> {getCategoryScore(item, 'Social') || 'N/A'}%
                                                                    </span>
                                                                </div>
                                                                <div className="col-12 ">
                                                                    <span className={` ${getScoreColor(getCategoryScore(item, 'Governance'))}`}>
                                                                        <strong>G:</strong> {getCategoryScore(item, 'Governance') || 'N/A'}%
                                                                    </span>
                                                                </div>
                                                            </div>
                                                        )}
...
export default UserMyAssessments;