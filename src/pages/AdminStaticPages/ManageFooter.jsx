...
function ManageFooter() {
    ...
    const fetchPage = async () => {
        try {
            setLoading(true);
            const res = await api.get('/static-pages/admin/footer');
            const data = res.data;
            setTitle(data.title || 'Footer');
            setContent(data.content || `© ${new Date().getFullYear()} Copyright SME Corp. Malaysia All rights reserved.`);
        } catch (error) {
            console.error('Error fetching page:', error);
            // If page doesn't exist, use default footer content
            setTitle('Footer');
            setContent(`© ${new Date().getFullYear()} Copyright SME Corp. Malaysia All rights reserved.`);
        } finally {
            setLoading(false);
        }
    };
    ...
    const handleSave = async () => {
        ...
        try {
            ...
            Swal.fire({
                icon: 'success',
                title: 'Saved!',
                text: 'Footer has been saved successfully.',
                timer: 1500,
                showConfirmButton: false
            });
        } catch (error) {
            console.error('Error saving page:', error);
            const errorMessage = error.response?.data?.detail || error.message || 'Failed to save footer.';
            Swal.fire({
                icon: 'error',
                title: 'Error',
                text: errorMessage
    ...
    if (loading) {
        return (
            <div className="container-fluid">
                <Title title="Manage Footer" breadcrumb={[["Pages", "/admin/pages"], "Footer"]} />
                    ...
                    <div className="card">
                        ...
                        <div className="card-body">
                            ...
                            {!isPreviewMode ? (
                                ...
                                <div className="border rounded p-4 bg-dark text-light" style={{ minHeight: '200px' }}>
